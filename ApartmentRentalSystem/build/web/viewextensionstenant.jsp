...
try {
    ...
    String sql =  "SELECT DATE_FORMAT(Renewal_Start_Date,'%d/%m/%Y'), DATE_FORMAT(Renewal_End_Date,'%d/%m/%Y'), DATE_FORMAT(Renewal_Request_Date,'%d/%m/%Y'), datediff(Renewal_Start_Date, Renewal_Request_Date), Renewal_Status, First_Name, Last_Name " +
    ...
    if(!rs.first()){ %>
        <h1>There is no lease extension request from you.</h1>
        <br><a href="tenantportal.jsp"> Back to Main Page</a>
        <% return;
    } else { rs.beforeFirst(); %>
    <h1>Lease Extension Requests</h1>
    <table border="2" width="100%" cellpadding="10" style="color: white; font-size:medium ; margin-top: 10px; ">
        <tr>
            <td>Renewal Start Date</td><td>Renewal End Date</td><td>Renewal Request Date</td><td>Prior Notice</td><td>Status</td><td>Tenant First Name</td><td>Tenant Last Name</td>
        </tr>

    <% while(rs.next()){ %>
        <tr>
            <td><%=rs.getString(1)%></td><td><%=rs.getString(2)%></td><td><%=rs.getString(3)%></td>
            <td><%=rs.getInt(4)%> days</td><td><%=rs.getString(5)%></td>
