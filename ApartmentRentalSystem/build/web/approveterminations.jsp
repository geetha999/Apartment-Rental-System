...
try {
    ...
    if(id[i] != null){
        st.executeUpdate("UPDATE Termination set  Termination_Status='Approved' WHERE Termination_ID = " + Integer.parseInt(id[i])); %>
        <h1>Termination Request Approved Successfully</font></h1>
        <% 
        String query = "select Apartment_Number from Termination WHERE Termination_ID = " + Integer.parseInt(id[i]) ;
        rs = st.executeQuery(query);
        String apartmentNumber = null ;
        if(rs.next()){ 
            apartmentNumber = rs.getString(1);
            System.out.println("Apartment number found." + apartmentNumber);
        }
        st.executeUpdate("UPDATE Apartment set  Apartment_Status = 'VACANT' WHERE Apartment_Number = '" + apartmentNumber + "'"); 
    ...
    if(!selectedRowFound){
        out.println("<h1> No Termination Request is selected to approve. </h1>");
        out.println();
    }
    }catch(Exception e){
...
%>
        <h1>
<br><a href="viewterminationsadmin.jsp"> Back to View Termination Requests</a>
