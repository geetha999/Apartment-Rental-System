...
try {
    ...
    if(id[i] != null){
        st.executeUpdate("UPDATE Renewal set  Renewal_Status='Rejected' WHERE Renewal_ID = " + Integer.parseInt(id[i])+ ""); %>
        <h1><font >Extension Rejected Successfully</font></h1>
    <% selectedRowFound = true ;
    } // end if
    }// end for
    if(!selectedRowFound){
        out.println("<h1> No Request is selected to reject. </h1>");
        out.println();
    }
    }catch(Exception e){
